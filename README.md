...
We have also conducted sample swap checks using microarray variant calls lifted over to our reference of interest by rsID using a dbSNP variant file corresponding to the reference. We have provided a Bash script run on Biowulf that lifts over rsIDs to the desired reference using a four step process: 
