...
Like merging, we annotate variant calls with different tools depending upon variant type. For example, we use AnnotSV for annotating structural variants and AnnoVar for annotating small nucleotide variants. We have included sample AnnotSV and AnnoVar scripts in the repository. We annotate SVs on a harmonized per sample basis (output of Sniffles merge and ```bcftools +split```) but SNVs on either a per sample or merged per cohort basis. We use the ```ensGene,ljb26_all,gnomad211_genome,clinvar_20190305``` databases with AnnoVar for SNV annotation.
