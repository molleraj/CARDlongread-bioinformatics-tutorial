...
## Basecalling
We basecall raw ONT data in POD5 format to unmapped BAMs using the ONT basecaller dorado. We currently use version 0.9.0 with the R10.4.1 E8.2 400bps super-accurate basecalling model v5.0.0 (```dna_r10.4.1_e8.2_400bps_sup@v5.0.0```). We plan to use dorado 1.0.0 or 1.1.1 in the future, pending comprehensive benchmarking. We also call per read 5mC/5hmC modifications in the process for modification pileups described later in the tutorial.
...
## Basecalling parallelization by flow cell channel
Since drafting our initial basecalling scripts, we have optimized a method for parallelizing basecalling call by flow channel described by ONT [here](https://software-docs.nanoporetech.com/dorado/latest/basecaller/duplex/#duplex-basecalling-performance) with the help of collaborators Elizabeth Hutchins and Daniel Enriquez at TGen/ASU. This involves converting input POD5 data from time series to separate files per channel, with complete read data for all reads sequenced in each channel (pore position of the flow cell) in each separate file. Basecalling is then run on some number (in our case, 32) of groups of channel POD5s and output unmapped BAMs are merged back into a single output unmapped BAM per run. We noted that we needed to use ```pod5``` version 0.3.27 for compatibility with the MinKNOW 24 API and to increase user limits (open files and process) above Biowulf defaults (from 4096 to 32768 in each case, with ```ulimit -n 32768 -u 32768```) in order to run the channel subsetting step with more than the default number of allocated additional threads. We have found this channel parallelization process reduces basecalling time from 12 hours to 4 hours for a 30x coverage genomic DNA sample given allocation of 4 A100 GPUs per basecalling job, given typical Biowulf HPC node availability. We then basecall quality filter merged output unmapped BAMs as otherwise and as described in the following section. 

The first step in preparing POD5s for channel subsetting is generating a table with the channel corresponding to each read with ```pod5 view``` as described above:
...
Channel group basecalling is then run on a sample sheet of pod5 channel subset directories (in this case, multiples of 32). It is run typically as an array job with the number of subjobs being the number of lines in the pod5 channel subset directory listing (e.g., ```sbatch --array=1-32%32 basecall_channel_groups_example.sh``` for a single sample subset into 32 chunks).

Upon further testing on the A100 GPU nodes of the NIH Biowulf HPC GPU partition, we found that allocating 4 A100 GPUs per job led to unpredictably long queue times, perhaps due to each node having a maximum of 4 A100 GPUs, and the job scheduler most likely prioritizing jobs requiring less than 4 A100 GPUs per job. We thus changed job constraints for channel group basecalling (```basecall_channel_groups_template.sh```) to 1 A100 GPU, 4 hours, and 20GB RAM per job (as well as 40GB local scratch space, while maintaining 30 CPUs per job). This change vastly reduced queue time in initial testing and resulted in roughly 600 jobs completing per day, equivalent to basecalling 15TB POD5s or ~10 30x WGS samples per day.
...
Depending upon cohort, we check for sample swaps (also called a concordance check) at both the initial flow cell and merged sample levels through whole genome alignment and variant calling. The sample swap calling procedure depends upon first subsetting samples' mapped reads per chromosome, calling single nucleotide variants from these subsets in relatively fast manner with Clair3, concatenating variant calls per sample, merging sample variant calls with corresponding short read variant calls, and then creating a kinship coefficient (KING) table with Plink2. We later expedited the variant merging and KING table generation processes by linkage disequilibrium (LD) pruning the merged short read joint variant call dataset and subsetting long read variant calls for those overlapping LD pruned short read joint calls. We have used a kinship coefficient of 0.375 (arithmetic mean between 0.25 and 0.5, corresponding to first degree siblings and identical individuals, respectively) as a cutoff for identifying identical pairs of samples either comparing long read against other long read variant calls or long read against short read variant calls.

We have also conducted sample swap checks using microarray variant calls lifted over to our reference of interest by rsID using a dbSNP variant file corresponding to the reference. We have provided a Bash script run on Biowulf that lifts over rsIDs to the desired reference using a five step process: 1) extracting rsIDs from microarray variant calls, 2) finding corresponding rsIDs in dbSNP desired reference version and pulling a table of chromosome, position, ID, reference, and alternate alleles with bcftools query, 3) using Plink to relabel chromosome and position of microarray calls, 4) using Plink to correct reference alleles at each position based on desired reference FASTA, and 5) running bcftools +fixref to correct remaining SNPs with mismatched strand (flip function). This script is provided as ```microarray_liftover_by_rsID_with_dbSNP.sh```.

```
Usage: ./microarray_liftover_by_rsID_with_dbSNP.sh -d [dbSNP VCF] -r [reference name, e.g., hg38] -f [reference FASTA for reference allele correction] -i [input microarray Plink bed file (.bed/.bim/.fam)] -o [output lifted over microarray variants (.vcf.gz)]
	-d Path to dbSNP VCF for reference of interest
	-f Path to target reference for reference allele correction
...
We have also begun to phase alignments using a combination of Clair3 to generate phased variant calls and WhatsHap to haplotag read alignments based on phased Clair3 calls. A sample Clair3/WhatsHap haplotagging script will be included in the near future. We also plan to compare performance of PMDV and Clair3/WhatsHap for haplotagging (e.g., average phasing block length).
# Variant calling
...
We call single nucleotide variants from long read alignments with several different tools, including Clair3 (https://github.com/HKU-BAL/Clair3), DeepVariant (https://github.com/google/deepvariant), and PEPPER-Margin-DeepVariant (PMDV) (https://github.com/kishwarshafin/pepper). We have found through testing that either flow cell or merged sample (multiple flow cells combined) level alignment data should be subsetted by each chromosome in order to increase job throughput on the NIH HPC cluster and maintain reasonable resource (especially RAM) use on the nodes of the norm partition. Template scripts for running each SNV caller (Clair3, DeepVariant, and PMDV) are included, as well as a template script for subsetting a mapped BAM alignment by chromosome to parallelize SNV calling and thus better utilize Biowulf shared resources (e.g., <80GB allocations on the norm partition). We use Clair3 variant calling models that correspond to the model used for basecalling that sample (e.g., ```r1041_e82_400bps_sup_v500```).
